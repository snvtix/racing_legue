...
TARGET_SPEED = 120  # Target speed in km/h. Lower this if you need a less aggressive setup.
STEER_GAIN = 40    # Steering sensitivity (original used 25 / PI).
CENTERING_GAIN = 0.4  # Correction toward track center (original used 0.25).
# Automatic braking before corners and high-speed overshoot.
BRAKE_THRESHOLD = 0.3
GEAR_SPEEDS = [0, 60, 100, 140, 190, 220]  # Gear shift thresholds for 1-6 in km/h.
ENABLE_TRACTION_CONTROL = True  # Keep traction control enabled to reduce wheelspin.
...
def calculate_throttle(S, R):
    steer_bias = abs(R['steer']) * 20.0
    target = TARGET_SPEED - steer_bias
    if S['speedX'] < target:
        accel = min(1.0, R['accel'] + 0.4)
    else:
        accel = max(0.0, R['accel'] - 0.3)
    if abs(R['steer']) > 0.35:
        accel = min(accel, 0.25)
    if S['speedX'] < 10:
...
def apply_brakes(S):
    ...
    if angle > BRAKE_THRESHOLD:
        brake = min(0.8, (angle - BRAKE_THRESHOLD) * 2.0 + 0.2)
    if S['speedX'] > TARGET_SPEED + 8:
        speed_brake = min(1.0, (S['speedX'] - TARGET_SPEED) / 35.0)
